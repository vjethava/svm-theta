function [RankedVariables,nbsv,values]=FeatSelalpha(x,y,c,epsilon,kernel,kerneloption,verbose,FeatSeloption)

% Usage
%  
% [RankedVariables,nbsv,values]=FeatSelalpha(x,y,c,kernel,kerneloption,verbose,FeatSeloption)
%
%
%   x,y     : input data
...
while length(SelectedVariables)~=0
    ...
    for i=1:length(SelectedVariables)
        ...
        if FeatSeloption.AlphaApprox 
           ...
           w2aux=(-0.5*alphaall(pos)'*Hnon(pos,pos)*alphaall(pos) +c(pos)'*alphaall(pos));
           w2(i)=w2aux;
...
%             w2aux=(-0.5*alphaaux(pos)'*Hnon(pos,pos)*alphaaux(pos) +c(pos)'*alphaaux(pos));
%             w2(i)=w2aux;
            w2(i)=sum(alphatemp);
